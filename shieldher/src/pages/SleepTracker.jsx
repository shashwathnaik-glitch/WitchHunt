...
const SleepTracker = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('shieldher_sleep');
    if (saved) {
      try {
      ...
      localStorage.setItem('shieldher_sleep', JSON.stringify(MOCK_SLEEP_DATA));
    }
  }, []);
  ...
  const handleSaveSleep = (e) => {
    ...
    localStorage.setItem('shieldher_sleep', JSON.stringify(newLogs));
      ...
      <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-8 pb-24">
        ...
        <div className="flex justify-between items-end">
          ...
          <button 
            ...
            className="min-h-[44px] min-w-[44px] px-4 py-2 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 transition-colors flex items-center gap-2 shadow-sm"
          >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-indigo-50">
                <h3 className="font-semibold text-indigo-900 text-lg">Log Your Sleep</h3>
                <button onClick={() => setIsModalOpen(false)} className="p-2 text-indigo-400 hover:text-indigo-600 hover:bg-indigo-100 rounded-full transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center">
                  <X className="w-5 h-5" />
                </button>
              </div>
              
              <form onSubmit={handleSaveSleep} className="p-6 space-y-8">
                
                <div className="flex items-center gap-4 bg-indigo-50/50 p-4 rounded-2xl border border-indigo-50">
                  <motion.div 
                    animate={{ rotate: [0, -10, 10, 0] }} 
                    transition={{ repeat: Infinity, duration: 4 }}
                    className="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 shadow-inner"
                  >
                    <Moon className="w-6 h-6" />
                  </motion.div>
                  <div className="flex-1">
                    <label className="block text-sm font-bold text-indigo-900 mb-1">Bedtime</label>
                    <input 
                      type="time" 
                      required
                      className="w-full bg-white border border-indigo-100 rounded-xl px-4 py-2 text-indigo-900 focus:ring-2 focus:ring-indigo-500 outline-none min-h-[44px] font-medium"
                      value={bedtime}
                ...
                <div className="flex items-center gap-4 bg-orange-50/50 p-4 rounded-2xl border border-orange-50">
                  <motion.div 
                    animate={{ rotate: 360 }} 
                    transition={{ repeat: Infinity, duration: 8, ease: "linear" }}
                    className="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center text-orange-500 shadow-inner"
                  >
                    <Sun className="w-6 h-6" />
                  </motion.div>
                  <div className="flex-1">
                    <label className="block text-sm font-bold text-orange-900 mb-1">Wake Time</label>
                    <input 
                      type="time" 
                      required
                      className="w-full bg-white border border-orange-100 rounded-xl px-4 py-2 text-orange-900 focus:ring-2 focus:ring-orange-500 outline-none min-h-[44px] font-medium"
                      value={wakeTime}
