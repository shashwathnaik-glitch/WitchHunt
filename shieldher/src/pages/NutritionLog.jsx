...
const NutritionLog = () => {
  ...
  useEffect(() => {
    const saved = loadData('nutrition_logs');
    if (saved) {
      if (saved.waterCups !== undefined) setWaterCups(saved.waterCups);
      if (saved.meals) setMeals(saved.meals);
    }
  }, []);

  const saveNutritionData = (newWater, newMeals) => {
    saveData('nutrition_logs', { waterCups: newWater, meals: newMeals });
  };
  ...
  const handleWaterClick = (index) => {
    let newCups = waterCups;
    if (index === waterCups) newCups = Math.min(8, waterCups + 1);
    else if (index < waterCups) newCups = index + 1;
    
    setWaterCups(newCups);
    saveNutritionData(newCups, meals);
  };
  ...
  const handleSaveFood = (e) => {
    ...
    const newMeals = {
      ...meals,
      [currentMealType]: [...meals[currentMealType], { id: Date.now(), ...foodEntry }]
    };
    
    setMeals(newMeals);
    saveNutritionData(waterCups, newMeals);
    ...
    <PageWrapper>
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {foodList.map(food => (
                            <tr key={food.id} className="hover:bg-gray-50 transition-colors">
                              <td className="px-4 py-3 font-medium text-gray-800 capitalize">{food.name}</td>
                              <td className="px-4 py-3 text-gray-600">{food.protein}g</td>
                              <td className="px-4 py-3 text-gray-600">{food.carbs}g</td>
                              <td className="px-4 py-3 text-gray-600">{food.fats}g</td>
                              <td className="px-4 py-3 text-right font-bold text-gray-700">{food.calories} kcal</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={`Add to ${currentMealType}`}>
        <form onSubmit={handleSaveFood} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">Search Food</label>
            <div className="relative">
              <Apple className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
              <input 
                type="text" 
                required
                className="w-full bg-black/30 border border-purple-500/30 text-white rounded-xl pl-10 pr-4 py-3 text-sm focus:ring-2 focus:ring-purple-500 outline-none min-h-[44px]"
                value={foodQuery}
          ...
          {foodEntry && (
            <div className="bg-purple-900/30 rounded-xl p-4 border border-purple-500/30">
              <p className="text-sm font-bold text-purple-400 mb-2 capitalize">{foodEntry.name} (1 serving)</p>
              <div className="grid grid-cols-4 gap-2 text-center text-xs">
                <div className="bg-black/40 rounded p-1 shadow-sm"><span className="block font-bold text-white">{foodEntry.calories}</span>kcal</div>
                <div className="bg-black/40 rounded p-1 shadow-sm"><span className="block font-bold text-white">{foodEntry.protein}g</span>Prot</div>
                <div className="bg-black/40 rounded p-1 shadow-sm"><span className="block font-bold text-white">{foodEntry.carbs}g</span>Carb</div>
                <div className="bg-black/40 rounded p-1 shadow-sm"><span className="block font-bold text-white">{foodEntry.fats}g</span>Fat</div>
              </div>
            </div>
          )}
          ...
          <div className="pt-4">
            <button 
              type="submit" 
              disabled={!foodEntry}
              className="w-full min-h-[44px] bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 transition-colors disabled:bg-gray-600 disabled:cursor-not-allowed"
            >
              Save Food
            </button>
          </div>
